...
function Funnel() {
  ...
  const demandSignals = ["цифровой след", "рыночные сигналы", "поиск решения", "активность"];
  const filters = [
    {
      label: "Сигнал спроса",
      note: "Компания проявляет признаки интереса к решению, а не просто подходит по отрасли.",
      icon: Target,
    },
    {
      label: "Роль ЛПР",
      note: "Выходим на людей, которые могут обсуждать задачу и влиять на решение.",
      icon: Users,
    },
    {
      label: "Потребность",
      note: "Проверяем, есть ли у компании реальная причина рассматривать продукт сейчас.",
      icon: Layers,
    },
    {
      label: "Готовность к диалогу",
      note: "Фиксируем только тот интерес, который можно передать вашей команде в работу.",
      icon: MessagesSquare,
    },
  ];
  const guaranteeLoop = [
    {
      label: "Разбираем причину",
      icon: RefreshCw,
    },
    {
      label: "Корректируем сегмент, оффер и аргументацию",
      icon: Target,
    },
    {
      label: "Запускаем следующий круг базы",
      icon: Layers,
    },
    {
      label: "Доводим результат до договорной гарантии",
      icon: ShieldCheck,
    },
  ];
  ...
  return (
    <div ref={ref} className="mx-auto w-full max-w-6xl">
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1.02fr_1.08fr_1.02fr] lg:items-stretch">
        <div className="relative overflow-hidden rounded-[32px] border border-border bg-background p-6 text-left">
          <div
            aria-hidden
            className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-axcend-action/30 blur-3xl"
          />
          <div className="relative flex h-full min-h-[340px] flex-col justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted/55 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
                <Layers className="h-3.5 w-3.5" />
                Вход
              </div>
              <div className="mt-8 text-6xl font-semibold tabular-nums tracking-normal text-foreground">
                500
              </div>
              <h3 className="mt-3 text-2xl font-semibold text-foreground">
                компаний с признаками спроса
              </h3>
              <p className="mt-5 max-w-sm text-base leading-relaxed text-muted-foreground">
                База собирается по цифровым сигналам, отраслевому контексту, активности на рынке и
                признакам поиска решения. Это не список “кому можно позвонить”, а отобранный
                сегмент, где есть коммерческий повод для разговора.
              </p>
            </div>
            <div className="mt-8 grid grid-cols-2 gap-2">
              {demandSignals.map((signal, idx) => (
                <div
                  key={signal}
                  className="flex items-center gap-2 rounded-full border border-border bg-card px-3 py-2 text-xs font-medium text-muted-foreground"
                  style={{ opacity: 0.72 + progress * 0.28, transitionDelay: `${idx * 90}ms` }}
                >
        ...
        <div className={`${DARK_SURFACE_BASE_CLASS} rounded-[32px] p-6 text-left`}>
          <div className="pointer-events-none absolute -right-36 top-10 h-80 w-80 rounded-full bg-axcend-action/20 blur-[92px]" />
          <div className="relative">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/12 bg-primary-foreground/8 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-primary-foreground/72">
              <ShieldCheck className="h-3.5 w-3.5" />
              AXCEND
            </div>
            <h3 className="mt-6 text-3xl font-semibold text-primary-foreground">
              Отбор и проверка интереса
            </h3>
            <p className="mt-4 text-base leading-relaxed text-primary-foreground/72">
              Мы не передаём “интересно, пришлите на почту”. В результат попадают только компании,
              где разговор прошёл квалификацию и его можно продолжать с заказчиком.
            </p>
              ...
              {filters.map((filter, idx) => {
                ...
                return (
                  <button
                    key={filter.label}
                    type="button"
                    onMouseEnter={() => setActiveFilter(idx)}
                    onFocus={() => setActiveFilter(idx)}
                    className={`group w-full rounded-2xl border p-4 text-left transition-all duration-300 ${
                      isActive
                        ? "border-axcend-action/70 bg-axcend-action text-axcend-dark"
                        : "border-primary-foreground/10 bg-primary-foreground/[0.045] text-primary-foreground hover:border-axcend-action/45"
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <span
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border transition-colors ${
                          isActive
                            ? "border-axcend-dark/10 bg-axcend-dark/10"
                      ...
                      <span>
                        ...
                        <span
                          className={`mt-1 block text-xs leading-relaxed ${
                            isActive ? "text-axcend-dark/72" : "text-primary-foreground/60"
                          }`}
                        >
                          {filter.note}
                        </span>
        ...
        <div className="relative overflow-hidden rounded-[32px] border border-axcend-action/55 bg-[#f5fdeb] p-6 text-left">
          <div
            aria-hidden
            className="absolute -bottom-20 -right-20 h-64 w-64 rounded-full bg-axcend-action/60 blur-[76px]"
          />
          <div className="relative flex h-full min-h-[340px] flex-col justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-axcend-dark/10 bg-background/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
                <Handshake className="h-3.5 w-3.5" />
                Выход
              </div>
              <div className="mt-8 flex items-end gap-2">
                <span className="text-7xl font-semibold tabular-nums tracking-normal text-foreground">
                  25
                </span>
                <span className="pb-2 text-4xl font-semibold text-foreground">+</span>
              </div>
              <h3 className="mt-3 text-2xl font-semibold text-foreground">предметных встреч</h3>
              <p className="mt-5 max-w-sm text-base leading-relaxed text-muted-foreground">
                Встречей считается только подтверждённый интерес: человек понял предложение и
                согласился обсуждать продукт с вашей командой.
              </p>
            </div>
            <div className="mt-8 rounded-3xl border border-axcend-dark/10 bg-background/72 p-4">
              <div className="flex items-center gap-3 text-sm font-semibold text-foreground">
                <ShieldCheck className="h-4 w-4 text-axcend-dark" />
                5% закреплены в договоре
              </div>
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                Минимальный результат известен заранее: из 500 компаний не менее 25 переходят в
                предметный разговор с заказчиком.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mt-5 overflow-hidden rounded-[28px] border border-border bg-background text-left">
        <div className="grid gap-0 lg:grid-cols-[0.72fr_1.28fr]">
          <div className="border-b border-border bg-muted/45 p-5 lg:border-b-0 lg:border-r">
            <div className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              Если 5% не достигнуты
            </div>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              Мы не закрываем проект отчётом. Корректируем работу и доводим результат до гарантии,
              закреплённой в договоре.
            </p>
          </div>
          <div className="grid grid-cols-1 divide-y divide-border md:grid-cols-4 md:divide-x md:divide-y-0">
            {guaranteeLoop.map((step, idx) => {
              const Icon = step.icon;
              return (
                <div key={step.label} className="relative p-5">
                  <div className="flex items-center gap-3 md:block">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-axcend-action/35 bg-axcend-action/20 text-axcend-dark">
                      <Icon className="h-4 w-4" />
                    </div>
                  ...
                  {idx < guaranteeLoop.length - 1 && (
                    <ArrowRight className="absolute -right-2 top-7 z-10 hidden h-4 w-4 rounded-full bg-background text-muted-foreground md:block" />
                  )}
                </div>
              );
            })}
  ...
  return (
    ...
    >
      ...
      <Section
        ...
        title="Не меньше 25 встреч с 500 компаний"
        center
      >
        <Funnel />
      </Section>
